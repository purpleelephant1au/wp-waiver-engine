...
class Settings {
    ...
    public function render(): void {
        ...
        $import_entries   = isset( $_GET['wpwe_import_entries'] ) ? absint( wp_unslash( $_GET['wpwe_import_entries'] ) ) : 0;
        $import_options   = isset( $_GET['wpwe_import_options'] ) ? absint( wp_unslash( $_GET['wpwe_import_options'] ) ) : 0;
        $handoff_deactivated = isset( $_GET['wpwe_handoff_deactivated'] ) ? absint( wp_unslash( $_GET['wpwe_handoff_deactivated'] ) ) : null;
        // phpcs:enable
        ...
        <div class="wrap">
            ...
            <div class="notice notice-info is-dismissible">
                ...
                <?php if ( $target !== '' ) : ?>
                <p>
                    <a class="button button-secondary" href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin.php?page=wpwe-settings&wpwe_action=deactivate_free_plugin&plugin=' . rawurlencode( $target ) ), 'wpwe_deactivate_free_plugin' ) ); ?>" onclick="return confirm('<?php echo esc_js( __( 'Deactivate the Free plugin now? Your data will remain in Pro.', 'waiver-engine' ) ); ?>');">
            ...
            <form method="post" action="<?php echo esc_url( admin_url( 'admin.php?page=wpwe-settings' ) ); ?>">
                ...
                <table class="form-table" role="presentation">
                    ...
                    <tr>
                        ...
                        <td>
                            <fieldset>
                                <label for="wpwe_captcha_provider"><?php esc_html_e( 'Provider:', 'waiver-engine' ); ?></label>
